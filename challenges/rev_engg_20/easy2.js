...
module.exports = {
    ...
    port: 2002,      // Netcat Port (Requested)
    webPort: 2672,   // Web Port (Hosts the Java file)
    ...
    // The integers below represent the flag characters packed into 32-bit ints.
...
class PollosLogistics {
     ...
     * To verify the shipment, you must reverse this packing logic.
     */
    ...
    public boolean verifyShipment(String code) {
        ...
        // These integers are the "packed" version of the password.
        // You need to unpack them back into ASCII characters.
    ...
    start: (socket, onWin) => {
        ...
        if (!module.exports.httpServer) {
            ...
            console.log(`[WEB] Starting Pollos Logistics Server on ${module.exports.webPort}`);
            ...
            module.exports.httpServer = http.createServer((req, res) => {
                ...
                res.end("<h1>Los Pollos Hermanos Logistics</h1><p>Status: Restricted</p><p>Manifest: <a href='/PollosLogistics.java'>PollosLogistics.java</a></p>");
            });
        ...
        socket.write("[+] INTEL: Gus packs the password into integers to hide it.\n");
        socket.write(`[+] DOWNLOAD: http://cookcrackcapture.in:${module.exports.webPort}/PollosLogistics.java\n`);
        socket.write("[+] MISSION: Reverse engineer the bit-packing logic to find the password.\n");
